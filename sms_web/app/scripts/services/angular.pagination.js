 ...
 */
smsWebApp
.directive('angularPagination', [ function(){
    return {
        restrict: 'EA',
        template: '<div class="page-list" ng-class="conf.floatAround"><div class="pagination">' +
            '<ul ng-show="conf.totalItems > 0">' +
            '<li ng-class="{disabled: conf.currentPage == 1}" ng-click="prevPage()"><a>&laquo;</a></li>' +
            '<li ng-repeat="item in pageList track by $index" ng-class="{active: item == conf.currentPage, separate: item == \'...\'}" ' +
            'ng-click="changeCurrentPage(item)"><a>{{ item }}</a></li>' +
            '<li ng-class="{disabled: conf.currentPage == conf.numberOfPages}" ng-click="nextPage()"><a>&raquo;</a></li>' +
            '</ul></div>' +
            '<div class="page-total" ng-show="conf.showTotal && conf.totalItems > 0">' +
        ...
        link: function(scope, element, attrs){
            ...
            if(!scope.conf.perPageOptions){
                scope.conf.perPageOptions = [10, 20, 30, 50, 100];
            }
            ...
            scope.$watch(function(){
                ...
                if(scope.conf.rememberPerPage){
                    ...
                    // 之所以不监控localStorage[scope.conf.rememberPerPage]是因为在删除的时候会undefined
                    // 然后又一次请求
